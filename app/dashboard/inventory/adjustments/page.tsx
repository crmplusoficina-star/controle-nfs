...
export default function InventoryAdjustmentsPage() {
  ...
  const savePhotos = async (files: FileList | File[] | null | undefined) => {
    ...
    try {
      ...
      const imageUrls = replacing
        ...
        : Array.from(new Set([...uploadedUrls].reverse().concat(oldUrls)));
  ...
  const selectableVisibleIds = useMemo(() => visibleTools
      ...
      {!visibleTools.length ? (
        ...
        <div className="grid gap-4">
          {visibleTools.map(tool => {
            ...
            return (
              <article key={tool.id} className={`relative bg-white border rounded-3xl shadow-sm overflow-hidden ${selectedIds.has(tool.id) ? 'ring-2 ring-rose-400 border-rose-200' : !draft.name.trim() ? 'border-amber-200' : 'border-slate-100'}`}>
